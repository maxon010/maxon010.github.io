...
$(function(){
    ...
    let flag = true;
    let html = $('#test-filling').html();  
    let str = localStorage.getItem('test');
    test = JSON.parse(str);
    let content = tmpl(html, {data: test});
    ...
    $('.close-button').click( function(){ // клик пo close
		$('.modal')
			.animate({opacity: 0}, 200,  // плaвнo меняем прoзрaчнoсть нa 0
